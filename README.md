# JARVIS SYSTEM — generated website deployment

**Official website:** [jarvisserver.org](https://jarvisserver.org)<br>
**Official shop:** [CORE SERIES 01](https://jarvisserver.org/shop/)<br>
**Founder and developer:** [Oliver-frank Pristaff](https://github.com/hotboxking09)<br>
**Support portal:** [Rules and transparency](https://jarvisserver.org/support/)<br>
**Reddit:** [u/JarvisSystemCH](https://www.reddit.com/user/JarvisSystemCH/)<br>
**Official support:** [One-time](https://ko-fi.com/jarvissystem) ·
[Memberships](https://ko-fi.com/jarvissystem/tiers)

This public repository contains only generated static website output. The full
authoring project and operating-system sources are not published here.

GitHub Pages publishes the repository root directly from `main`; no custom
Actions workflow is required for this already generated static output.

Copyright © 2026 JARVIS SYSTEM rights holder. All rights reserved. No reuse
licence and no operating-system ISO are provided. See `LICENSE.md` and
`RIGHTS_AND_RELEASE_POLICY.md`.

Support is voluntary, is not an investment, and grants no ownership, team,
administrative, system, or governance rights.
